...
public class AtAction extends BaseAction{
	...
	public String index() {
		...
		// 获取微博列表
		Pair<DataPage<Weibo>,Map<String,User>> pair = WeiboService.getAtWbs(u.getId(), ApplicationConstant.pageSize, 1);
	...
	public String more() {
		...
		// 获取微博列表
		Pair<DataPage<Weibo>,Map<String,User>> pair = WeiboService.getAtWbs(u.getId(), ApplicationConstant.pageSize, pageNo);
		DataPage<Weibo> dp = pair.getFirst();
		List<Weibo> wbs = dp.getRecords();
